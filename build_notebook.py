"""Build OPTIMIZED MARec notebook — ~1 hour runtime on Colab Pro.

Key optimizations vs original:
- HP grid: 4 combos (was 64) — we already know lambda1=100, beta=1 works best
- Tune on 1 split (was 3)
- Evaluate on 5 splits (was 10) — still enough for CI + paired t-test
- Pre-compute features once for no_tags configs (shared similarity matrices)
"""
import json, os

script_dir = os.path.dirname(os.path.abspath(__file__))
output_path = os.path.join(script_dir, "MARec_Fast_Experiments.ipynb")
...
# ===== TITLE =====
cells.append(md("""# MARec++: Fast Comprehensive Experiment Suite
...
cells.append(md("## Core Functions (data, features, similarity, EASE, evaluation)"))
...
# ===== PREDICTION + EXPERIMENT RUNNER =====
cells.append(md("## Experiment Runner (optimized)"))

cells.append(code("""def predict(train_csr, feat_names, ds, tag_mode, hp, min_count=2, shuffle_tags=False):
...
# ===== LOAD DATA =====
cells.append(md("## Load Data & Generate Splits"))

cells.append(code("""print("Loading dataset...")
...
# ===== TAG STATS =====
cells.append(md("## Tag Statistics"))

cells.append(code("""tag_df = ds["dfs"]["tags"]; ut_df = ds["dfs"]["user_tags"]
...
# ===== ALL EXPERIMENTS =====
cells.append(md("""## Run ALL Experiments
...
# ===== RESULTS TABLE =====
cells.append(md("## Comprehensive Results Table"))

cells.append(code("""from tabulate import tabulate
...
# ===== SIGNIFICANCE TESTS =====
cells.append(md("## Statistical Significance Tests"))

cells.append(code("""bl_scores = [m.get("hr@10",0) for m in results["top3_no_tags"]["per_split"]]
sig_rows = []
for n in order:
...
cells.append(code("""# Per-split variance
...
cells.append(code("""# Tag mode comparison (side by side)
...
cells.append(code("""out = Path("results/fast_experiments"); out.mkdir(parents=True, exist_ok=True)
rows_csv = []
